...
% Calculate and display interest over time
calculateAndPlotInterest(rawSignal, fs, windowSize, overlap);

function calculateAndPlotInterest(signal, fs, windowSize, overlap)
    ...
    % Calculate interest index for each window
    for i = 1:numWindows
        ...
        interestIndex(i) = betaPower / alphaPower;
    ...
    % Show results
    figure;
    plot(timePoints, interestIndex, 'Color', [0.0, 0.5, 0.0], 'LineWidth', 2);
    title('Interest Index Over Time');
    xlabel('Time (s)');
    ylabel('Interest Index (Beta/Alpha Power Ratio)');
    
    % Add reference lines
    hold on;
    yline(mean(interestIndex), 'r--', 'Average Interest', 'LineWidth', 1.5);
    yline(mean(interestIndex) + std(interestIndex), 'g--', 'High Interest', 'LineWidth', 1.5);
    yline(mean(interestIndex) - std(interestIndex), 'b--', 'Low Interest', 'LineWidth', 1.5);
    hold off;
    
    % Set chart range
    ylim([0, max(interestIndex) * 1.1]);
    
    % Add description
    legend('Interest Index', 'Average', 'High Interest', 'Low Interest');
    grid on;
end